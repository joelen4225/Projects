...
function emptyInputSignup($name, $email, $user, $pass, $pass2)
{
    $result;
    
    if(empty($name) || empty($email) || empty($user) || empty($pass) || empty($pass2))
    {
        $result = true;
    }
    else
...
function createUser($conn, $user, $email, $pass)
{
    ...
    mysqli_stmt_bind_param($stmt, "ssss", $name, $user, $email, $hashPass);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
